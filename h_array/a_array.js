...
// join() : 전달한 값으로 Array 객체의 각 요소를 구분한 뒤 문자열로 리턴한다.
...
// [중요] forEach(callback) : 전달한 callback 함수의 매개변수에 값, 인덱스, 원본객체를 전달한다.
// console.log(datas);

// 순서상 매개변수를 작성해야 하지만, 사용하지 않는다면, _(언더바)를 작성한다.
...
// [ 중요 ] map(callback) : 기존 값을 callback함수의 리턴값으로 변경시킨다.
// 작업이 완료된 Array객체를 리턴한다.
...
// [ 중요 ] filter(callback) : 전달한 callback함수의 리턴 값이 true일 때에만 해당 값 추출한다.
// let datas = [10, 20, 15, 30, 25];
...
// 두 번째 매개변수 : Array객체의 각 값 (요소)
// 세 번째 매개변수 :두 번째에 매개변수에 들어온 값의 인덱스
// 네 번째 매개변수 : 원본 Array 객체

// reduce(callback, initValue)
// 만약 initValue를 작성한다면, 첫 번째 매개변수에 initValue가 들어간다.

// const result2 = datas.reduce((variable, data, i) => {
//     return 3;
// });
// console.log(result2);
...
// const result5 = datas.reduce((variable, _, i) => {
//     variable[i] = i + 1;
//     return variable;
// }, []);
// console.log(result5);

// let string_datas = ["안", "녕", "하", "세", "요"];
// const result6 = string_datas.reduce((variable, data) => {
//     return data + variable;
// });
// console.log(result6);

// datas = [1, 2, 3, 4, 5];
// const result7 = datas.reduce((variable, data, i) => {
//     variable[`data${i + 1}`] = `${i}.박은혜`;
//     return variable;
// }, {});

// console.log(result);

// new Array(확보해놓을 공간 수 ) : 미리 칸 수를 확보해 놓을 수 있다.
// 하지만, 공간만 확보되었고 값은 아무것도 들어가 있지 않기 때문에
...
// 처음부터 공간을 확보하고 싶을 때, but 값이 없기 때문에 공간이 있는건 아니다.
// undefined 조차도 없다. forEach나 map하면 0번 돈다.
// 그래서 fill이용. 20칸에 다 0이라는 값이 들어감
